    ...
    stages {
        stage('Notify Teams') {
          steps {
            ...
            def data = """{"@type":"MessageCard","@context":"http://schema.org/extensions","themeColor":"0072C6","title":"Jenkins Build Alert","text":"${currentBuild.resultIsBetterOrEqualTo('SUCCESS') ? 'Successful' : 'Failed'}","potentialAction":[{"@type":"OpenUri","name":"${BUILD_NUMBER}cppcheckResult/","targets":[{"os":"default","uri":"${BUILD_URL}"}]}]}"""
