    ...
    stages {
        stage('Notify Teams') {
          steps {
            ...
            def data2 = """{"@type":"MessageCard","@context":"http://schema.org/extensions","themeColor":"0072C6","title":"Jenkins Build","text":"Alert","potentialAction":[{"@type":"OpenUri","name":"jenkins","targets":[{"os":"default","uri":"${BUILD_URL}"}, {"name": "Build Number", "value": "${BUILD_NUMBER}"},                 {"name": "Status", "value": "${currentBuild.resultIsBetterOrEqualTo('SUCCESS') ? 'Successful' : 'Failed'}]}]}"""
