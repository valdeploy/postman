    ...
    stages {
        stage('Notify Teams') {
          steps {
            ...
            def data2 = """{"@type":"MessageCard","@context":"http://schema.org/extensions","themeColor":"0072C6","title":"Jenkins Build","text":"Alert","potentialAction":[{"@type":"OpenUri","name":"jenkins","targets":[{"os":"default","build":"${BUILD_NUMBER}","uri":"${PROJECT_URL}"}]}]}"""
